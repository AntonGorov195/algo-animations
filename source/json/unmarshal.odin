...
Unmarshal_Custom_Proc :: #type proc(
	self: ^Custom_Unmarshal,
	p: ^Parser,
	v: any,
	end_token: Token_Kind,
) -> Unmarshal_Error
Custom_Unmarshal :: struct {
	type:      typeid,
	unmarshal: Unmarshal_Custom_Proc,
	data:      rawptr,
}
...
custom_unmarshals: [dynamic]Custom_Unmarshal
...
unmarshal_any :: proc(
	...
	if p.spec == .MJSON {
		...
		case .Ident, .String:
			if is_unmarshaled(&p, v, .EOF) or_return {
				return nil
			}
...
unmarshal_value :: proc(p: ^Parser, v: any) -> (err: Unmarshal_Error) {
	...
	if is_unmarshaled(p, v, .Close_Brace) or_return {
		return nil
	}
	v := v
	...
	if u, ok := ti.variant.(reflect.Type_Info_Union); ok && token.kind != .Null {
		...
		tag_field_value := parse_value(p) or_return
		parse_comma(p)
...
unmarshal_object :: proc(p: ^Parser, v: any, end_token: Token_Kind) -> (err: Unmarshal_Error) {
	...
	case reflect.Type_Info_Struct:
		...
		struct_loop: for p.curr_token.kind != end_token {
			...
			if field_found {
				field_test :: #force_inline proc "contextless" (
					field_used: [^]byte,
					offset: uintptr,
				) -> bool {
