...
Marshal_Custom_Proc :: #type proc(
	self: ^Custom_Marshal,
	w: io.Writer,
	v: any,
	opt: ^Marshal_Options,
) -> Marshal_Error
Custom_Marshal :: struct {
	data:    rawptr,
	type:    typeid,
	marshal: Marshal_Custom_Proc,
}
...
custom_marshals: [dynamic]Custom_Marshal
...
marshal_to_writer :: proc(w: io.Writer, v: any, opt: ^Marshal_Options) -> (err: Marshal_Error) {
	...
	is := is_marshaled(w, v, opt) or_return
	if is {
		return nil
	}
	...
	case runtime.Type_Info_Rune:
		...
		if r == '\x00' {
			io.write_byte(w, '\\') or_return
			io.write_byte(w, 'u') or_return
			io.write_byte(w, '0') or_return
			io.write_byte(w, '0') or_return
			io.write_byte(w, '0') or_return
			io.write_byte(w, '0') or_return
		} else {
	...
	case runtime.Type_Info_Struct:
		is_omitempty :: proc(v: any) -> bool {
			...
			case runtime.Type_Info_Union, runtime.Type_Info_Bit_Set, runtime.Type_Info_Soa_Pointer:
		...
		marshal_struct_fields :: proc(
			...
			for name, i in info.names[:info.field_count] {
				...
				json_name, extra := json_name_from_tag_value(
					reflect.struct_tag_get(reflect.Struct_Tag(info.tags[i]), "json"),
				)
...
is_marshaled :: proc(w: io.Writer, v: any, opt: ^Marshal_Options) -> (bool, Marshal_Error) {
	for &custom in custom_marshals {
		if custom.type == v.id {
			return true, custom->marshal(w, v, opt)
		}
	}
	return false, nil
}
