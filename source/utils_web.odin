...
foreign import local_storage_lib "local_storage"

local_storage_set :: proc(key, value: string) {
	@(default_calling_convention = "contextless")
	foreign local_storage_lib {
		_write_local_storage :: proc(key, value: string) ---
	}
	_write_local_storage(key, value)
}
local_storage_get :: proc(key: string, allocator := context.allocator) -> string {
	@(default_calling_convention = "contextless")
	foreign local_storage_lib {
		_read_local_storage_length :: proc(key: string) -> int ---
		_read_local_storage :: proc(key: string, buf: []byte) ---
	}
	length := _read_local_storage_length(key)
	buf := make([]u8, length, allocator = context.temp_allocator)
	_read_local_storage(key, buf)
	return string(buf)
}
